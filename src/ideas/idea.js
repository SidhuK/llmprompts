...
async function initializeSearch() {
  try {
    ...
    const isDevMode =
      document.getElementById("audienceSelect").value === "developers";
    ...
    .then((csvText) => {
      ...
      promptElements.forEach((blockquote) => {
        ...
        <div class="prompt-title">
            ...
            <button class="yaml-button" title="Show prompt.yml format" onclick="showYamlModal(event, '${encodeURIComponent(
              title
...
function buildPrompt(encodedPrompt) {
  ...
  const tone =
    toneSelect.value === "custom" ? customTone.value : toneSelect.value;
...
function showYamlModal(event, title, content) {
  ...
  const convertedContent = cleanContent.replace(
    /\${([^:}]+)(?::[^}]*)?}/g,
    (match, varName) => {
      return `{{${varName.trim()}}}`;
    }
...
messages:
  ...
  const highlightedYaml = yamlText
    .replace(
      /(name|model|modelParameters|temperature|messages|role|content):/g,
  ...
  createYamlButton.addEventListener("click", () => {
    ...
    const filename =
      cleanTitle.toLocaleLowerCase().replace(/\s+/g, "-") + ".prompt.yml";
